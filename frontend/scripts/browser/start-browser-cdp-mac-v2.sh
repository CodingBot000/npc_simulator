...
find_playwright_chromium() {
  find "$PLAYWRIGHT_CACHE" -maxdepth 7 -type f 2>/dev/null | rg '/(Chromium|chrome|Google Chrome for Testing)$' | sort | tail -n 1
}
...
echo "If this still fails, run npm run browser:install once and retry." >&2
echo "Log: $LOG_PATH" >&2
exit 1
