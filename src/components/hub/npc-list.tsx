...
export function NpcList({ npcs, selectedNpcId, onSelect }: NpcListProps) {
  return (
    <Panel
      eyebrow="Hub Cast"
      title="NPC 목록"
      subtitle="마을 허브에 있는 인물들의 현재 상태"
      className="h-full"
    >
            ...
            <button
              ...
              className={`w-full rounded-[24px] border px-4 py-4 text-left transition ${
                selected
                  ...
                  : "border-[var(--panel-border)] bg-white/40 hover:border-[var(--teal)] hover:bg-white/60"
              }`}
            >
              ...
              <div className="mt-3 flex items-center justify-between text-xs font-medium text-[var(--ink-muted)]">
                ...
                <span>신뢰 {npc.relationship.playerTrust}</span>
              </div>
            </button>
          );
        })}
